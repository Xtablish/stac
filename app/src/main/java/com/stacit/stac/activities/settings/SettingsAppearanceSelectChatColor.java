...
import com.stacit.stac.databinding.ActivitySettingsAppearanceSelectChatColorBinding;

public class SettingsAppearanceSelectChatColor extends AppCompatActivity {
    private ActivitySettingsAppearanceSelectChatColorBinding binding;
    private PreferenceManager preferenceManager;
    private int backgroundColor = 0;
    private String chatColorSelection = null;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivitySettingsAppearanceSelectChatColorBinding.inflate(getLayoutInflater());
    ...
    private void chatColorSelect()
    {
        binding.chatColor1.setOnClickListener(view -> {
            backgroundColor = ContextCompat.getColor(getApplicationContext(), R.color.chat_bubble_default);
            chatColorSelection = "chat_default_color";
        ...
        binding.chatColor10.setOnClickListener(view -> {
            ...
            chatColorSelection = "chatColor10";
    ...
    {
        Drawable previewDrawable = binding.textSentMessage.getBackground();
        previewDrawable = DrawableCompat.wrap(previewDrawable);
        //the color is a direct color int and not a color resource
        DrawableCompat.setTint(previewDrawable, backgroundColor);
        binding.textSentMessage.setBackground(previewDrawable);

        preferenceManager.putString(Constants.KEY_CHAT_BUBBLE_COLOR, Integer.toString(backgroundColor));
        preferenceManager.putString(Constants.KEY_CHAT_COLOR_SELECTION, chatColorSelection);
    ...
    {
        if (preferenceManager.getString(Constants.KEY_CHAT_BUBBLE_COLOR) != null && preferenceManager.getString(Constants.KEY_CHAT_COLOR_SELECTION) != null)
        {
            Drawable previewDrawable = binding.textSentMessage.getBackground();
            previewDrawable = DrawableCompat.wrap(previewDrawable);
            //the color is a direct color int and not a color resource
            ...
            showLastSelection(preferenceManager.getString(Constants.KEY_CHAT_COLOR_SELECTION));
        }
    }
